...
from useful_functions import get_argument_dict, get_domain_class
...
if __name__ == "__main__":
    ...
    argParser.add_argument("-o","--order_object_placement",    help = "set to remove random order of planks", action="store_false")
    ...
    argParser.add_argument("-c","--objects_in_init_state",    help = "objects_in_init_state",                nargs="?",default=0)
    argParser.add_argument("-q","--quadrant_number",          help = "quadrant to use",                      nargs="?")
    argParser.add_argument("-m","--minimum_object_count",      help = "minimum number of objects to use",     nargs="?")
    ...
    order_object_placement = args.order_object_placement
    ...
    minimum_object_count = args.minimum_object_count
    ...
    if minimum_object_count is not None:
        minimum_object_count = int(minimum_object_count)
    ...
    for env_num,name in enumerate(env_list):
        ...
        if false_load_arguments:
            thismodule = sys.modules[__name__]
            argument_dict = get_argument_dict(module_name=MOD_NAME,problem_name=name)
            for key in argument_dict.keys():
