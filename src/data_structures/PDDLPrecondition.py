...
class LiftedPDDLPrecondition(object):
    ...
    def get_grounded_precondition(self,grounding,ll_state_object_list): 
        true_set = set()
        false_set = set() 
        for prop in self.true_set: 
            true_set.add(prop.ground_relation(grounding,ll_state_object_list))
        for prop in self.false_set: 
            false_set.add(prop.ground_relation(grounding,ll_state_object_list))
    ...
    def __str__(self):
        ...
        for a_prop in self.sort_set(self.true_aux_set):
            if a_prop.id <= 2:
                ...
                if str(a_prop).split()[1].split("_")[Config.OBJ_TYPE_IND] in Config.CONST_TYPES:
