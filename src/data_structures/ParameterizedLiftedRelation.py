...
class ParameterizedLiftedRelation(object):
    ...
    def ground_relation(self,grounding,ll_state_object_list): 
        if "Const" in self.pid1:
            grounded_param1 = Link(link_name=self.pid1,link_type=self.pid1.split("_")[Config.OBJ_TYPE_IND])
        else:
            grounded_param1 = grounding[self.pid1]
            
        if "Const" in self.pid2:
            grounded_param2 = Link(link_name=self.pid2,link_type=self.pid2.split("_")[Config.OBJ_TYPE_IND])
        else:
            grounded_param2 = grounding[self.pid2]

        return self.parent_relation.get_grounded_relation(grounded_param1, grounded_param2, ll_state_object_list)    

    def __str__(self):
        if self.parent_relation.parameter1_type in Config.CONST_TYPES:
        ...
        elif self.parent_relation.parameter2_type in Config.CONST_TYPES:
            return "({}_{}_{} ?{} {})".format(self.parent_relation.parameter1_type, self.parent_relation.parameter2_type,self.parent_relation.cr, self.pid1, self.pid2)
